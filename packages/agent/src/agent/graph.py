...
InitDoc → ProductSupport → ParseParams → END
"""
...
def _should_continue(state: dict) -> str:
    """Route after init_doc: skip downstream nodes if errored."""
...
def create_pipeline_graph() -> CompiledStateGraph:
    ...
    Flow:
    InitDoc → [error → END |
               ProductSupport → ParseParams → END]
    ...
    graph.add_edge("product_support", "parse_params")
    graph.add_edge("parse_params", END)
