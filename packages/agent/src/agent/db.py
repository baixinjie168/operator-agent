...
def delete_runs_by_task_name_prefix(prefix: str) -> dict:
    """Delete all pipeline_runs rows whose ``task_name`` starts with ``prefix``.

    Used by the batch-task mirror to clean up per-item rows when the user
    deletes a batch via ``DELETE /api/v1/tasks/{id}`` — these rows are
    inserted with ``task_name = f"[batch {task_id}] {operator_name}"`` and
    have no ``parent_task_id`` link to a parent row (each row carries the
    UUID generated by ``RuntimeManager.create_run``), so the regular
    ``delete_task`` cascade cannot find them.

    Cascades to ``exec_results``, ``test_cases`` and ``pipeline_events`` for
    each deleted run, mirroring the behaviour of ``delete_task``.
    """
    db = get_db()
    rows = db.conn.execute(
        "SELECT run_id FROM pipeline_runs WHERE task_name LIKE ?",
        (f"{prefix}%",),
    ).fetchall()
    run_ids = [r[0] for r in rows]
    if not run_ids:
        return {"deleted_tasks": 0, "task_ids": []}

    deleted = 0
    for rid in run_ids:
        db.conn.execute("DELETE FROM exec_results WHERE task_id = ?", (rid,))
        db.conn.execute("DELETE FROM test_cases WHERE task_id = ?", (rid,))
        db.conn.execute("DELETE FROM pipeline_events WHERE run_id = ?", (rid,))
        db.conn.execute("DELETE FROM pipeline_runs WHERE run_id = ?", (rid,))
        deleted += 1

    db.conn.commit()
    logger.info(
        "Deleted %d batch mirror rows with task_name prefix %r", deleted, prefix
    )
    return {"deleted_tasks": deleted, "task_ids": run_ids}
