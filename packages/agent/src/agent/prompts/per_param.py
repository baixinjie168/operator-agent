...
- 如果参数名包含 Optional，默认该参数为可选；除非描述中明确出现"必选"、"必须"、"不可为空"等字眼，才返回 false
- 如果参数名不含 Optional，则只看描述内容中是否出现上述关键词，出现则返回 true，否则返回 false
...
你是一个数组长度提取专家。从下面参数的描述文本中，提取该数组参数的长度/大小约束，并解析为 [最小值, 最大值] 区间。

提取规则:
1. 只关注描述中关于数组长度、大小、元素个数的约束
2. 常见表达: "列表长度不超过N"、"最大长度为N"、"size大小为N"、"维度为N"、
   "支持的最大TensorList长度为N"、"长度范围[M, N]" 等
3. value 统一解析为二元整数列表 [min, max]：
   - 固定长度 N（如"长度为2"、"shape为(2)"、"维度为3"）→ [N, N]
   - 范围 [M, N]（如"长度范围[2,16]"）→ [M, N]
   - 仅上限 N（如"最大长度256"、"不超过N"）→ [0, N]
4. src_text 保留原文中描述长度的完整文字（用于溯源）
5. 如果描述中没有任何长度相关信息，value 设为 null，src_text 设为空字符串
6. value 中的数值必须是整数，不要带单位或文字

严格按以下JSON格式返回，不要添加任何其他文字:
{{"value": [2, 2], "src_text": "1维，shape为(2)，即长度为2"}}

仅上限示例: {{"value": [0, 256], "src_text": "若不为空时可支持的最大长度为256个"}}
无长度信息示例: {{"value": null, "src_text": ""}}
