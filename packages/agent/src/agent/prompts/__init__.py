...
你是一个参数描述专家。从下面的文档内容中，为参数 "{param_name}" 生成一段逻辑完整的描述文本，
并同时提取对应的原始文本作为溯源依据。

要求：
1. 仔细阅读文档内容，提取与参数 "{param_name}" 直接相关的所有信息
2. 用自然语言重新组织这些信息，确保每句话主语明确（以参数名为主语）
3. 不要简单复制原文，而是理解后用完整语句重新表达
4. 只包含**平台无关的通用属性**，即所有平台都适用的约束
5. 覆盖以下通用属性（如果文档中有相关信息）：
   - 参数的角色（输入/输出）
   - 参数的功能描述
   - 通用数据类型（dtype，所有平台一致的部分）
   - 通用数据格式（format，所有平台一致的部分）
   - 通用维度约束（shape，所有平台一致的部分）
   - 是否支持非连续 Tensor
   - 通用使用条件和约束（无平台前缀）
   - 是否可选（必选/可选）
   - 通用取值范围（无平台前缀）
