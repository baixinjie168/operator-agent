...
要求：
1. 每个函数对应一个对象
2. parameter 列表中包含该函数的所有参数（包括输入和输出参数），每个参数包含 name（参数名）和 type（C类型）
3. type 只保留基础类型名，去掉指针符号 *，如 aclTensor* → aclTensor，uint64_t* → uint64_t
...
def _flatten_to_parameters(functions: list[dict]) -> list[dict]:
    ...
    for func in functions:
        ...
        for param in func.get("parameter", []):
            if isinstance(param, str):
                param_name = param
                param_type = ""
            else:
                param_name = param.get("name", "")
                param_type = param.get("type", "").rstrip("*").strip()
            parameters.append({
