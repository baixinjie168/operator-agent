...
_SYSTEM_PROMPT = """\
你是一个昇腾算子测试专家。根据提供的 CPU Golden Reference 推导指南，\
修改更新 ATK 算子测试执行 py 文件，补充 cpu_golden_reference 函数的实现。

要求：
1. 根据算子名称匹配推导指南中对应的 CPU 实现
2. 替换 cpu_golden_reference 中的 NotImplementedError 为实际实现
3. 保持文件其他部分不变
4. 只输出修改后的完整 Python 文件代码，不要包含 markdown 代码块标记
"""
...
async def exec_cpu_derivation_node(state: PipelineState) -> dict[str, Any]:
    ...
    try:
        ...
        user_prompt = (
            f"请根据以下 aclnn_cpu_golden_derivation.md 推导指南，"
            f"帮我修改更新 ATK 的 {operator_name}_atk_executor.py 算子测试执行 py 文件。\n\n"
            f"## 推导指南\n{skill_content}\n\n"
