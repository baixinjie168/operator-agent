...
- 每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnBaddbmmGetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnBaddbmm”接口执行计算。
```cpp
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <li>shape需要与batch1@batch2的后两维的shape满足<a href="../common/broadcast关系.md">broadcast关系</a>（注意：只能是self通过broadcast变成和batch1@batch2的后两维一样的shape，不可以batch1@batch2的后两维通过broadcast变成和self一样，举例：self：[2, 3, 5]，batch1@batch2：[1, 1, 1]，会报错。）。</li>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>公式中的输入batch1。</td>
      <td><ul><li>数据类型与self、batch2的数据类型需满足数据类型推导规则（参见<a href="../common/互推导关系.md">互推导关系</a>和<a href="#约束说明">约束说明</a>）。</li><li>shape需要与batch2满足bmm输入约束关系。</li><li>支持空Tensor。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>ND</td>
      <td>3</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>公式中的输入batch2。</td>
      <td><ul><li>数据类型与self、batch1的数据类型需满足数据类型推导规则（参见<a href="../common/互推导关系.md">互推导关系</a>和<a href="#约束说明">约束说明</a>）。</li><li>shape需要与batch1满足bmm输入约束关系。</li>。<li>支持空Tensor。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>ND</td>
      <td>3</td>
      <td>√</td>
