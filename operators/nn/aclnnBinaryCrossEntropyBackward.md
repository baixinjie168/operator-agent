...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnBinaryCrossEntropyBackwardGetWorkspaceSize”接口获取入参并根据流程计算所需workspace大小，再调用“aclnnBinaryCrossEntropyBackward”接口执行计算。
...
- **参数说明：**
    ...
    | out | 输出 | 存储梯度计算结果，shape与self相同。 | [数据格式](../common/数据格式.md)需要与self一致。|-|ND|-|√|
