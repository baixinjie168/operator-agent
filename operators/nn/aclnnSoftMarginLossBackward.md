...
算子功能：计算[aclnnSoftMarginLoss](./aclnnSoftMarginLoss.md)二分类逻辑损失函数的反向传播。reduction指定损失函数的计算方式，支持 'none'、'mean'、'sum'。'none' 表示不应用reduction，'mean' 表示输出的总和将除以输出中的元素数，'sum' 表示输出将被求和。

## 函数原型

每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnSoftMarginLossBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSoftMarginLossBackward”接口执行计算。
...
- **参数说明**：
  ...
  * gradOutput（aclTensor*, 计算输入）：Device侧的aclTensor。shape需要与self、target满足[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
  ...
  * self（aclTensor*, 计算输入）：Device侧的aclTensor。shape需要与gradOutput、target满足[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
    - <term>Atlas 训练系列产品</term>：数据类型支持FLOAT16、FLOAT。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT16、FLOAT、BFLOAT16。
  * target（aclTensor*, 计算输入）：Device侧的aclTensor。shape需要与gradOutput、self满足[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
    - <term>Atlas 训练系列产品</term>：数据类型支持FLOAT16、FLOAT。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT16、FLOAT、BFLOAT16。
  ...
  * out（aclTensor*, 计算输出）：Device侧的aclTensor。shape需要是target与self、gradOutput满足broadcast之后的shape，详见[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
    - <term>Atlas 训练系列产品</term>：数据类型支持FLOAT16、FLOAT。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT16、FLOAT、BFLOAT16。
