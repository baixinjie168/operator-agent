...
- **参数说明：**
  ...
  - gradOutput(aclTensor\*, 计算输入)：当前节点的梯度。Device侧的aclTensor，且数据类型与self一致，NC维度需要保持一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND、NCHW、NCL，且[数据格式](../common/数据格式.md)需要与self一致。NCL输入时按照(C, Hin, Win)处理。
  ...
  - self(aclTensor *, 计算输入)：输入张量，叶子节点。Device侧的aclTensor。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND、NCHW、NCL。NCL输入时按照(C, Hin, Win)处理。
    - <term>Atlas 训练系列产品</term>、<term>Atlas 推理系列产品</term>：数据类型支持FLOAT16、FLOAT32。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT32。

  - out (aclTensor\*, 计算输出)：输出张量，对应了输入叶子节点的梯度。Device侧的aclTensor，shape与self一致，[数据格式](../common/数据格式.md)支持ND、NCHW、NCL，且[数据格式](../common/数据格式.md)与self一致。NCL输入时按照(C, Hin, Win)处理。
    - <term>Atlas 训练系列产品</term>、<term>Atlas 推理系列产品</term>：数据类型支持FLOAT16、FLOAT32。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持BFLOAT16、FLOAT16、FLOAT32。
