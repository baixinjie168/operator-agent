...
**该接口后续版本会废弃，请使用最新接口[aclnnQuantMatmulV4](./aclnnQuantMatmulV4.md)，接口迁移方法参考本文档[约束说明](#约束说明)。**
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/matmul/quant_matmul)
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnQuantMatmulGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnQuantMatmul”接口执行计算。
```cpp
...
- **参数说明**
  ...
  <tbody>
    <tr>
      <td>x1</td>
      <td>输入</td>
      <td>维度与x2一致，不支持broadcast。</td>
      <td>数据类型需要与x2满足<a href="../common/互推导关系.md">互推导关系</a></td>
      <td>INT8</td>
      <td>ND</td>
      <td>2-3</td>
      <td>-</td>
    ...
    <tr>
      <td>x2</td>
      <td>输入</td>
      <td>维度与x1一致，不支持broadcast。</td>
      <td>数据类型需要与x1满足<a href="../common/互推导关系.md">互推导关系</a></td>
      <td>INT8</td>
      <td>ND</td>
      <td>2-3</td>
      <td>-</td>
    ...
    <tr>
      <td>bias</td>
      <td>输入</td>
      <td>shape支持一维(n, )，n与x2的n一致。公式输入bias。</td>
    ...
    <tr>
      <td>out</td>
      <td>输出</td>
      <td>Shape需要是x1与x2经过推导之后的Shape</td>
      <td>-</td>
      <td>FLOAT16</td>
      <td>ND</td>
      <td>-</td>
