...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnRepeatInterleaveTensorGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnRepeatInterleaveTensor”接口执行计算。
...
- **参数说明：**
  ...
  - repeats(aclTensor*, 计算输入)：重复的次数。Device侧的aclTensor，数据类型支持INT32、INT64。repeats为1D Tensor（包括shape=[0,]的场景），支持空tensor。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
