...
接口功能：完成[aclnnPreluBackward](./aclnnPreluBackward.md)的反向函数。
gradInput的计算公式如下：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnPreluBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnPreluBackward”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>dtype需要与self相同。</li><li>shape需要与self满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>，且Broadcast后shape与self的shape相等。</li></ul></td>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>为self的梯度值。</td>
      <td><ul><li>dtype需要与self相同。</li><li>shape需要与gradOutput满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>。</li><li>gradInput的shape和数据类型与self的相同。</li></ul></td>
