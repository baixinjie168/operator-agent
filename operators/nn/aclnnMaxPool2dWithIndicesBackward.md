...
- 算子功能：
正向最大池化[aclnnMaxPool2dWithIndices](./aclnnMaxPool2dWithIndices.md)的反向传播。
- 输入tensor的推导公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnMaxPool2dWithIndicesBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMaxPool2dWithIndicesBackward”接口执行计算。
...
- **参数说明：**
  * gradOutput(const aclTensor \*, 计算输入): 反向传播过程中上一步输出的梯度，Device侧aclTensor。和正向的输出shape一致。支持[非连续的Tensor](../common/非连续的Tensor.md)。数据格式与self保持一致。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT。[数据格式](../common/数据格式.md)支持NCHW和CHW。
  * self(const aclTensor \*, 计算输入): 正向的输入数据，Device侧aclTensor。支持[非连续的Tensor](../common/非连续的Tensor.md)。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型支持FLOAT。[数据格式](../common/数据格式.md)支持NCHW和CHW。
  * indices(aclTensor \*, 计算输入): 正向输出的索引，是Device侧aclTensor，shape和输入gradOutput一致。正向输出中最大元素的索引位置，数据格式与self保持一致。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型仅支持INT32。[数据格式](../common/数据格式.md)支持NCHW和CHW。
  ...
  * gradInput(aclTensor \*, 计算输出): 反向传播输出的梯度，是Device侧aclTensor。shape与self保持一致。数据格式与self保持一致。
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：数据类型仅支持FLOAT。[数据格式](../common/数据格式.md)支持NCHW和CHW。
