...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnTransQuantParamV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnTransQuantParamV2”接口执行计算。
...
- <term>Atlas 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：该接口支持与matmul类算子（如[aclnnQuantMatmulV4](./aclnnQuantMatmulV4.md)）配套使用。
...
- 关于scale、offset、out的shape说明如下：
  - 当无offset时，out shape与scale shape一致。
    - 若out作为matmul类算子输入（如[aclnnQuantMatmulV4](./aclnnQuantMatmulV4.md)），shape支持1维(1,)、(n,)或2维(1, n)，其中n与matmul计算中右矩阵（对应参数x2）的shape n一致。
  ...
  - 当有offset时，仅作为matmul类算子输入（如[aclnnQuantMatmulV4](./aclnnQuantMatmulV4.md)）：
