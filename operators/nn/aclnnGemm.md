...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/matmul/gemm)
...
- 接口功能：计算α 乘以A与B的乘积，再与β 和input C的乘积求和。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnGemmGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnGemm”接口执行计算。
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      ...
      <li>shape需要与A@B计算后的结果 一致或满足<a href="../common/broadcast关系.md">broadcast关系</a>。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>ND</td>
      <td>2</td>
      <td>√</td>
    ...
    <tr>
      <td>out</td>
      <td>输出</td>
      <td>公式中的out，数据类型需要与C构成互相推导关系，shape需要A@B计算后的结果一致。</td>
      <td>-</td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>ND</td>
      <td>2</td>
