...
* **参数说明**：
  ...
  * out(aclTensor \*, 计算输出): 输出Tensor，是Device侧aclTensor。池化后的结果。数据类型与self保持一致。shape由上述公式推导出。[数据格式](../common/数据格式.md)支持ND，与self保持一致。
  * indices(aclTensor \*, 计算输出): 输出Tensor，是Device侧aclTensor。最大值的索引位置组成的Tensor。数据类型仅支持INT32。shape和out保持一致。[数据格式](../common/数据格式.md)支持ND。
