...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnNanMedianGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnNanMedian”接口执行计算。
...
- **参数说明：**
  ...
  + out(aclTensor*, 计算输出)：Device侧的aclTensor，数据类型与self相同，out为0维Tensor。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
    - <term>Atlas 训练系列产品</term>：数据类型支持FLOAT、FLOAT16、UINT8、INT8、INT16、INT32、INT64。
