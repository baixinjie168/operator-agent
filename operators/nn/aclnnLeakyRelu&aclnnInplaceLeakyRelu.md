...
- 每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnLeakyReluGetWorkspaceSize”或者“aclnnInplaceLeakyReluGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnLeakyRelu”或者“aclnnInplaceLeakyRelu”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>out的数据类型需要是self可转换的数据类型（参见<a href="../common/互转换关系.md" target="_blank">互转换关系</a>）。</td>
      <td>FLOAT、FLOAT16、BFLOAT16、DOUBLE</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
