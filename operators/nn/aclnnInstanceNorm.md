...
- 接口功能：用于执行Instance Normalization（实例归一化）操作。与[aclnnBatchNorm](./aclnnBatchNorm.md)相比，aclnnInstanceNorm在每个样本的实例上进行归一化，而不是在整个批次上进行归一化，这使得该函数更适合处理图像等数据。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用`aclnnInstanceNormGetWorkspaceSize`接口获取入参并根据计算流程所需workspace大小，再调用`aclnnInstanceNorm`接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>数据类型与输入`x`的数据类型保持一致。</li><li>shape与输入`x`满足<a href="../common/broadcast关系.md">broadcast关系</a>（前2维的shape和输入x前2维的shape相同，前2维表示不需要norm的维度，其余维度大小为1）。</li></ul></td>
      <td>FLOAT32、FLOAT16</td>
      <td>ND</td>
      <td>4</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>数据类型与`x`的数据类型保持一致。</li><li>shape与输入`x`满足<a href="../common/broadcast关系.md">broadcast关系</a>（前2维的shape和输入x前2维的shape相同，前2维表示不需要norm的维度，其余维度大小为1）。</li></ul></td>
