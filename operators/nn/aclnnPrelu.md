...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnPreluGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnPrelu”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
      ...
      <td><ul><li>支持空Tensor。</li><li>数据类型需要与weight满足数据类型推导规则（参见<a href="../common/互推导关系.md" target="_blank">互推导关系</a>）。</li></ul></td>
