...
- **参数说明：**
  ...
  - gradOutput（aclTensor \*， 计算输入）：梯度Tensor，Device侧aclTensor，数据类型支持FLOAT32、FLOAT16、BFLOAT16。和正向的输出shape一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND， 当输入是5维时，内部按照NCDHW处理，当输入是4维时，在0维度处补1，内部按照NCDHW处理。
    
  - self（aclTensor \*， 计算输入）：正向的输入Tensor，Device侧aclTensor，数据类型支持FLOAT32、FLOAT16、BFLOAT16。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND， 当输入是5维时，内部按照NCDHW处理，当输入是4维时，在0维度处补1，内部按照NCDHW处理，与gradOutput一致。
    
  - indices（aclTensor \*， 计算输入）：输入Tensor，是Device侧aclTensor，数据类型仅支持INT32。正向输入中最大元素的索引位置。[数据格式](../common/数据格式.md)与gradOutput保持一致。shape与gradOutput一致
