...
- 算子功能：
  新增aclnnSilentCheckV2接口，该版本与[aclnnSilentCheck](./aclnnSilentCheck.md)接口的计算逻辑做了改变，根据stepRef参数判断val与新增马尔可夫不等式阈值进行对比校验。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnSilentCheckV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSilentCheckV2”接口执行计算。
