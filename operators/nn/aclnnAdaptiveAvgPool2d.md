...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnAdaptiveAvgPool2dGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAdaptiveAvgPool2d”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>输出</td>
      <td>Device侧的aclTensor。</td>
      <td>NCL输入时按照(C, Hout, Wout)处理。</td>
      <td>BFLOAT16、FLOAT16、FLOAT32</td>
      <td>NCHW、NCL</td>
      <td>由self和outputSize共同指定</td>
      <td>-</td>
    </tr>
    <tr>
      <td>workspaceSize</td>
