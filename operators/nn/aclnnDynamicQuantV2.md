...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnDynamicQuantV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDynamicQuantV2”接口执行计算。
...
- **参数说明：**
  ...
  - <term>Atlas 推理系列产品</term>、<term>Atlas 训练系列产品</term>：
    - 入参`x`仅支持FLOAT16。
    - 入参`smoothScalesOptional`、`groupIndexOptional`只支持为nullptr。
    - 入参`dstType`只支持配置为2。
    - 出参`yOut`只支持INT8。
