...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/index/scatter_elements)
...
- 算子功能：[aclnnMaxPool](./aclnnMaxPool.md)在2d的逆运算，由outputSize决定out的H、W轴大小，并根据indices索引在out中填入self的元素值，其余位置都设置为0。
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnMaxUnpool2dGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMaxUnpool2d”接口执行计算。
...
- **参数说明：**
  ...
  - self(aclTensor\*, 计算输入): 公式中的输入`self`, Device侧的aclTensor。数据类型支持FLOAT、FLOAT16、INT16、INT32、INT64、INT8、UINT8和DOUBLE，且数据类型与out的数据类型一致，shape与indices保持一致。支持[非连续的Tensor](../common/非连续的Tensor.md),[数据格式](../common/数据格式.md)支持ND和NCHW，维度支持3-4维，当维度为3时，各维度依次表示C、H、W，当维度为4时，各维度依次表示N、C、H、W，其中C代表channel, N代表Batch，H代表Height，W代表Width。
  - indices(aclTensor\*, 计算输入): 公式中的输入`indices`, Device侧的aclTensor。表示输入self的元素在输出结果中的索引位置。数据类型支持INT64、INT32，且shape与self保持一致。支持[非连续的Tensor](../common/非连续的Tensor.md),[数据格式](../common/数据格式.md)支持ND和NCHW，维度支持3-4维，当维度为3时，各维度依次表示C、H、W，当维度为4时，各维度依次表示N、C、H、W，其中C代表channel, N代表Batch，H代表Height，W代表Width。
  - outputSize(aclIntArray*, 计算输入):表示输出结果在H和W维度上的空间大小，Host侧的aclIntArray。size大小为2, 两个元素乘积值需大于等于self在H和W维度上的size乘积值。
  - out(aclTensor\*, 计算输出):公式中的输出`out`, Device侧的aclTensor。数据类型支持FLOAT、FLOAT16、INT16、INT32、INT64、INT8、UINT8和DOUBLE，且数据类型与self的数据类型一致。支持[非连续的Tensor](../common/非连续的Tensor.md),[数据格式](../common/数据格式.md)支持ND和NCHW，维度支持3-4维，当维度为3时，各维度依次表示C、H、W，当维度为4时，各维度依次表示N、C、H、W，其中C代表channel, N代表Batch，H代表Height，W代表Width。
