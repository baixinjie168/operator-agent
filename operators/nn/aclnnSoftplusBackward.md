...
- 接口功能：[aclnnSoftplus](./aclnnSoftplus.md)的反向传播。
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnSoftplusBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSoftplusBackward”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>shape需要与self、gradInput相同。</li><li>shape需要与self满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>，且Broadcast后shape与self的shape相等。</li></ul></td>
    ...
    <tr>
      ...
      <td><ul><li>数据类型满足数据类型推导规则（参见<a href="../common/互推导关系.md" target="_blank">互推导关系</a>）。</li><li>支持可转换为FLOAT的类型。</li></ul></td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
