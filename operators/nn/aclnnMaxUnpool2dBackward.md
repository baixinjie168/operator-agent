...
算子功能：MaxPool2d的逆运算[aclnnMaxUnpool2d](./aclnnMaxUnpool2d.md)的反向传播，根据indices索引在out中填入gradOutput的元素值。
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnMaxUnpool2dBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMaxUnpool2dBackward”接口执行计算。
...
- **参数说明：**
  ...
  - gradOutput(aclTensor*, 计算输入)：公式中的输入`gradOutput`，Device侧的aclTensor，数据类型支持FLOAT、FLOAT16、INT32、INT64、INT16、INT8、UINT8、BOOL，数据类型需要可转换为out的数据类型（参见[互转换关系](../common/互转换关系.md)），且数据类型需要与self一致，shape需要为(N, C, outputSize[0], outputSize[1])或(C, outputSize[0], outputSize[1])，维度需要与self一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。

  - self(aclTensor*, 计算输入)：Device侧的aclTensor，数据类型支持FLOAT、FLOAT16、INT32、INT64、INT16、INT8、UINT8、BOOL，且数据类型需要与gradOutput一致，shape需要为(N, C, H, W)或(C, H, W)，且shape需要与indices一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。

  - indices(aclTensor*, 计算输入)：公式中的输入`indices`，Device侧的aclTensor，数据类型支持INT64、INT32，shape需要与self一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。

  - outputSize(aclIntArray*, 计算输入)：Host侧的aclIntArray，元素个数必须为2，元素值必须与gradOutput的shape的最后两维一致。

  - out(aclTensor*, 计算输出)：公式中的`out`，Device侧的aclTensor，数据类型支持FLOAT、FLOAT16、INT32、INT64、INT16、INT8、UINT8、BOOL，shape需要与self一致。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
