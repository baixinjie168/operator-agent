...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/index/top_k_top_p_sample)
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用`aclnnTopKTopPSampleGetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnTopKTopPSample`接口执行计算。
