...
- 接口功能：[aclnnHardshrink](./aclnnHardshrink.md)的反向接口，计算反向传播的梯度gradInput。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnHardshrinkBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnHardshrinkBackward”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
      ...
      <td><ul><li>gradOutput与self的shape满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>。</li><li>gradOutput与self、gradInput的数据类型一致。</li></ul></td>
