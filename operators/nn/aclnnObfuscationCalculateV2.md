...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用 “aclnnObfuscationCalculateV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnObfuscationCalculateV2”接口执行计算。
...
* **参数说明**：
  ...
  * x（aclTensor* ，计算输入）：待混淆处理的张量，Shape为(*,*,...,hiddenSize)，最后一维的size是aclnnObfuscationSetupV2在资源初始化时配置的hiddenSize，[数据格式](../common/数据格式.md)要求为ND。不支持[非连续的Tensor](../common/非连续的Tensor.md)，不支持空Tensor。
...
- 该接口与[aclnnObfuscationSetupV2](./aclnnObfuscationSetupV2.md)配套使用，完成PMCC模型混淆功能，使用方式如下：
