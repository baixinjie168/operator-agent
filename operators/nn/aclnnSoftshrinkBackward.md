...
- 接口功能：完成[Softshrink](./aclnnSoftshrink.md)函数的反向接口。
- 计算公式：
  - Softshrink正向计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnSoftshrinkBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSoftshrinkBackward”接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>shape与self满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>。</li><li>gradOutput、self和gradInput的数据类型一致。</li></ul></td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>正向输入值。公式中的x。</td>
      <td><ul><li>支持空Tensor。</li><li>shape与gradOutput满足<a href="../common/broadcast关系.md" target="_blank">broadcast关系</a>。</li><li>数据类型需要与gradOutput一致。</li></ul></td>
      <td>FLOAT16、FLOAT、BFLOAT16</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
