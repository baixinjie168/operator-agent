...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/matmul/fused_linear_online_max_sum)
...
- 接口功能：
  ...
  功能等价Megatron的matmul与fused\_vocab\_parallel\_cross\_entropy的实现，支持vocabulary\_size维度切卡融合matmul与celoss，中间根据通信拆分为[aclnnFusedLinearOnlineMaxSum](./aclnnFusedLinearOnlineMaxSum.md)、[aclnnFusedCrossEntropyLossWithMaxSum](./aclnnFusedCrossEntropyLossWithMaxSum.md)，需要依次调用实现完整功能。
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnFusedLinearOnlineMaxSumGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnFusedLinearOnlineMaxSum”接口执行计算。
  ...
  <thead>
   ...
   <tbody>
     ...
     <tr>
      ...
      <td><ul><li>数据类型需要与input一致。</li><li>shape为[input.size(0), weight.size(0)]。</li><li>当vocabParallelLogitsOutOptional为nullptr时为省显存分支，否则为高性能分支。</li></ul></td>
      <td>BFLOAT16、FLOAT16</td>
      <td>ND</td>
      <td>2</td>
      <td>√</td>
