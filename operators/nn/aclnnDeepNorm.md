...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnDeepNormGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDeepNorm”接口执行计算。
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>shape与输入`x`满足<a href="../common/broadcast关系.md">broadcast关系</a>（前几维的维度和输入`x`前几维的维度相同，前几维表示不需要norm的维度，其余维度大小为1）。</li></ul></td>
      <td>FLOAT32</td>
      <td>ND</td>
      <td>2-8</td>
      <td>√</td>
