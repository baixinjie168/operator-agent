...
- 接口功能：对由多个输入通道组成的输入信号应用最近邻插值算法进行上采样。如果输入shape为(N, C, L)，则输出shape为(N, C, outputSize)。本接口相较于[aclnnUpsampleNearest1d](aclnnUpsampleNearest1d.md)，增加入参scaleL，请根据实际情况选择合适的接口。
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnUpsampleNearest1dGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnUpsampleNearest1d”接口执行计算。
