...
- 接口功能：[aclnnUpsampleNearest3d](aclnnUpsampleNearest3d.md)的反向计算。
- 计算公式：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用`aclnnUpsampleNearest3dBackwardGetWorkspaceSize`接口获取入参并根据计算流程计算所需workspace大小，再调用`aclnnUpsampleNearest3dBackward`接口执行计算。
