...
- 接口功能：[aclnnUpsampleBicubic2d](aclnnUpsampleBicubic2d.md)的反向传播。如果输入张量的shape为(N, C, H, W)，则输出张量的shape为(N, C, inputSize[2], inputSize[3])。
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>不支持空Tensor。</li><li>数据类型与`gradOut`一致。</li><li>out的所有维度取值均小于等于(2^31-1)。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>NCHW、ND、NHWC</td>
      <td>4</td>
      <td>√</td>
