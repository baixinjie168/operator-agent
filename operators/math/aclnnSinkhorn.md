...
[📄 查看源码](https://gitcode.com/cann/ops-math/tree/master/math/sinkhorn)
...
- **参数说明**：
    ...
    - cost（aclTensor*，计算输入）：表示成本张量，公式中的`cost`，Device侧的aclTensor。数据类型支持BFLOAT16、FLOAT16、FLOAT。取值需要在[0,1]之间，可以进行归一化处理。 [数据格式](common/数据格式.md)支持ND，输入为二维矩阵且行数不超过10000，列数不超过1024。支持[非连续的Tensor](common/非连续的Tensor.md)。    
...
- **返回值**：
  ...
  aclnnStatus：返回状态码，具体参见[aclnn返回码](./aclnn返回码.md)。
