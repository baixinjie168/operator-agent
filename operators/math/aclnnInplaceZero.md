...
算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnInplaceZeroGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnInplaceZero”接口执行计算。
...
- **参数说明**：
  ...
  - selfRef(aclTensor*, 计算输入|计算输出)：公式中的`selfRef`，Device侧的aclTensor，支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
