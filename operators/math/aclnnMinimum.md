...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnMinimumGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMinimum”接口执行计算。
...
- **参数说明：**
  ...
  - self(aclTensor*, 计算输入)：Device侧的aclTensor，数据类型与other的数据类型需满足数据类型推导规则（参见[互推导关系](../common/互推导关系.md)），shape需要与other满足[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
  ...
  - other(aclTensor*, 计算输入)：Device侧的aclTensor，数据类型与self的数据类型需满足数据类型推导规则（参见[互推导关系](../common/互推导关系.md)），shape需要与self满足[broadcast关系](../common/broadcast关系.md)。支持[非连续的Tensor](../common/非连续的Tensor.md)，[数据格式](../common/数据格式.md)支持ND。
