...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>compressInfo</td>
      <td>输入</td>
      <td>数据类型为INT64。其中包括压缩块信息tilingN、tilingK（通过msModelSlim工具中weight_compression模块压缩后获取，分别表示压缩前shape(n, k)在n方向和k方向上一个基本压缩块的大小），压缩前x2矩阵原始shape（shape为2维，用(n, k)表示），以及压缩块遍历方向的标识。</td>
      <td>-</td>
      <td>INT64</td>
      <td>-</td>
      <td>-</td>
