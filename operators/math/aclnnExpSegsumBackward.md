...
- 接口功能：[aclnnExpSegsum](aclnnExpSegsum.md)的反向计算。
- 计算公式（以5D输入为例）：
...
每个算子分为[两段式接口](../common/两段式接口.md)，必须先调用“aclnnExpSegsumBackwardGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnExpSegsumBackward”接口执行计算。
