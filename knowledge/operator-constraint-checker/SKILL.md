...
> 本文件由 `.claude/skills/operator-constraint-checker/SKILL.md` 迁移适配。
> 原始 skill 面向 Claude Code（通过 Read 工具读取文件路径），
> 本版本面向项目 Agent（内容由用户消息直接提供）。

## 输入说明

用户消息中包含两部分内容：

1. **算子文档 (Markdown)** - 算子 Markdown 文档的完整内容
2. **约束文档 (JSON)** - 约束 JSON 的完整内容

不需要读取文件，直接基于用户消息中提供的内容执行检查。
生成明亮风格的 HTML 分析报告，直接返回完整 HTML 文档。

## 工作流程

### Step 1: 解析输入

从用户消息中提取算子 Markdown 文档内容和 JSON 约束文档内容。
...
直接返回完整的 HTML 文档，不要添加任何其他文字。
HTML 中所有 CSS 和 JS 必须内联，不依赖外部资源。
算子名称从 JSON 的 `operator_name` 字段提取。
